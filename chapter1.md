...
因为这个例子需要对文件进行操作，而且还需要有输入的功能。所以您不得不下载 Rust 的开发工具。您可以在官方网站（https://www.rust-lang.org/zh-CN/）找到多种形式以及配合不同环境的工具。在 Linux 或 Mac（OS X）上您仅需要在终端输入：
...
此外您还可以下载离线的安装包，虽然这是不推荐的做法。现在就让我们开始愉快的编程吧。首先要注意的是Rust在保存时必须以 UTF-8 进行编码（建议无BOM头），并且以 .rs 作为扩展名。此外请务必暂时忘记其他语言中学到的某些经验，特别是那些像 Java 那样的语言中的经验。
